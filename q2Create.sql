...
CREATE MATERIALIZED VIEW ActiveStudents AS select distinct sr.studentid from StudentRegistrationsToDegrees sr where NOT EXISTS(SELECT * FROM CompletedStudentRegistrations WHERE StudentRegistrationId = sr.StudentRegistrationId);
