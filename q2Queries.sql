...
select 0;
select 0;
SELECT degreeid, birthyearstudent, gender, avg(avgMark)::float as avgGrade FROM students s natural join passedcreditsperregistration pc natural join studentregistrationstodegrees sr natural join activestudents ar natural join courseregistrations cr natural join courseoffers co natural join courses c  GROUP BY CUBE(degreeid, birthyearstudent, gender)  ORDER BY degreeid, birthyearstudent, gender;
